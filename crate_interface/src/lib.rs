...
pub fn call_interface(item: TokenStream) -> TokenStream {
    parse_call_interface(item).unwrap_or_else(
        |msg| compiler_error(Error::new(Span::call_site(), msg))
    )
}

fn parse_call_interface(item: TokenStream) -> Result<TokenStream, String> {
    let mut iter = item.into_iter();
    let tt = iter.next_chunk::<4>().or(Err("expect `Trait::func`"))?
        .map(|t| t.to_string());
    ...
    Ok(call.parse::<TokenStream>().or(Err("expect a correct argument list"))?)
}
